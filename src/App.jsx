...
function App() {
  ...
  // Exponer una función global para verificar filas de programación contra la DB
  useEffect(() => {
    const verify = async (rows) => {
      try {
        ...
        for (const row of rows) {
          ...
          // Si se proporcionó tipo, comprobar que coincida con alguna programación de la misma ruta
          if (tipo) {
          ...
          // Validar cantidad de corridas si está disponible
          if (numeroCorridas !== null) {
            const matchWithCorridas = matchesByRuta.find(p => {
              ...
              // Si hay tipo, validar que coincida también
